...
- **M-2. Remove inspector shell-COM lock probe + cancellation correctness (absorbs B3, B9).** Delete the file-identity interop wrapper and `IFileIsInUse` / `SHCreateItemFromParsingName`; `NtfsFileIdentityService` uses Restart Manager directly for lock diagnostics. Explicit `catch (OperationCanceledException) { throw; }` in the seven `NtfsFileIdentityService.*Async` methods. Expected diff: ~150 lines.
