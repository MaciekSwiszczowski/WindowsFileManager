...
**Status:** in progress — code complete on `master`, manual verification (§8.4) outstanding
...
- **Finalize U-4.** Complete the §8.4 manual checklist on a Windows 11 workstation (100% + 150% DPI). Capture results below under "Acceptance results" once run. When the checklist is fully green, switch this note's status from `in progress` to `complete`.
- **Roadmap slot 4 — `SPEC_KEYBOARD_SHORTCUTS_GAPS.md`** is unblocked for its `F2` / `Shift+F6` items (K-2): the in-cell rename surface they depend on is in place. `FileEntryTableView.OnPreviewKeyDown` already wires both keys; K-2 can focus on the remaining gaps (Alt+Up, Alt+Enter, Shift+Delete) plus migrating the registry.
- **Keyboard-shortcuts baseline doc.** Two edits fall to K-2 (per `SPEC_KEYBOARD_SHORTCUTS_GAPS.md` §5): `winui-file-manager-keyboard-shortcuts-spec.md` §12.9 heading → "Rename in place (`F2`, `Shift+F6`)"; §17 table Rename row → `F2, Shift+F6`. Do not do them in U-4's wrap-up.
...
- [ ] §8.4 `F2` on a file → stem selected, commit on Enter, focus returns to the row (manual).
- [ ] §8.4 `Esc` cancels; original name restored (manual).
- [ ] §8.4 LostFocus (click-away) commits (manual).
- [ ] §8.4 invalid-character name keeps edit mode open and does not hit the filesystem (manual).
- [ ] §8.4 collision keeps edit mode open with the disk entry unchanged (manual; automated coverage already green, but the UX path needs eyeballing).
- [ ] §8.4 `Shift+F6` behaves identically to `F2` (manual).
- [ ] §8.4 `F2` on `..` does nothing (manual).
- [ ] §8.4 toolbar "Rename" triggers in-cell edit (manual).
...
- [ ] §8.7 regression pass over `winui-file-manager-keyboard-shortcuts-spec.md` §17 (manual).
- [ ] §8.1 / §8.2 / §8.3 from the prior batches re-run once with U-4 in place to confirm no regressions in splitter / columns / persistence (manual).
